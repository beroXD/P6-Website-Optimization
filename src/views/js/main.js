...
var resizePizzas = function (size) {
	...
	function changePizzaSizes(size) {
		...
		for (var i = 0; i < randomPizzas.length; i++) {
			...
			// instead of px we use % to not waste time on conversion
		}
	}
...
var pizzasDiv = document.getElementById("randomPizzas"); // move outside loop so that it is called one time
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
	...
	var topScroll = document.body.scrollTop / 1250; //move outside for loop
	var phase = [];
	for (var i = 0; i < 5; i++) {
...
document.addEventListener('DOMContentLoaded', function () {
	var s = 256;
	// screen.height and screen.width property returns the height and width of the visitor's screen in pixels
	// https://www.w3schools.com/js/js_window_screen.asp
	var height = window.screen.height;
	var width = window.screen.width;
	var rows = Math.floor(height / s) + 1;
	var cols = Math.floor(width / s) + 1;
	pizzaOnScreen = cols * rows; // number of pizza to fill the screen
	var elem; // creating elem variable outside loop
	for (var i = 0; i < pizzaOnScreen; i++) {
		...
		// replaced querySelector with getElementById for better performans
